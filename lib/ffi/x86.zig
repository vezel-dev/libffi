...
const arg_longlong = if (builtin.cpu.arch == .x86_64) switch (builtin.abi) {
    .gnux32, .muslx32 => true,
    else => builtin.os.tag == .windows,
} else false;
...
pub const Abi = if (builtin.os.tag == .windows)
    if (builtin.cpu.arch == .x86_64) enum(i32) {
        ...
        pub const default: Abi = if (builtin.abi == .msvc or builtin.abi == .itanium) .win64 else .gnuw64;
    } else enum(i32) {
        sysv = 1,
        stdcall = 2,
        thiscall = 3,

...
const version = std.SemanticVersion.parse("3.4.6") catch unreachable;
...
pub fn build(b: *std.Build) anyerror!void {
    ...
    const fficonfig_h = b.addConfigHeader(.{
        ...
        .FFI_EXEC_STATIC_TRAMP = switch (t.os.tag) {
            .linux => if (t.cpu.arch.isArm() or t.cpu.arch.isAARCH64() or t.cpu.arch.isLoongArch() or t.cpu.arch.isX86()) true else null,
            else => null,
        },
        ...
        .PACKAGE_NAME = "libffi",
        .PACKAGE_STRING = "libffi " ++ version_str,
    ...
    example.root_module.addImport("ffi", ffi_mod);
